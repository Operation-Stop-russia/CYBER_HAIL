...
Echo.
Echo 	введiть "3"  щоб виконати патч для старту сеансiв з командою встановлення та запуску mhddos_proxy
Echo			     з використанням IP машин CloudShell (Тест!)
Echo.
Echo.
...
if "%choice%"=="3" Copy /Y Bin\ssh.py "C:\Program Files (x86)\Google\Cloud SDK\google-cloud-sdk\lib\googlecloudsdk\command_lib\util\ssh" && Copy /Y Bin\3\Command(Operation-Stop_russia).txt C:\Windows\Temp && echo Test! Patch GCloudSDK для mhddos_proxy (--vpn) встановлено && pause && exit
